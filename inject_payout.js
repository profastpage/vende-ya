...
// 1. Import requestPayout and toast
text = text.replace(
  /import \{ cn \} from '@\/lib\/utils'/,
  "import { cn } from '@/lib/utils'\nimport { requestPayout } from './actions'\nimport { toast } from 'sonner'"
);

// 2. Add state for PayoutModal
text = text.replace(
  /function DashboardContent\(\) \{/,
  "function DashboardContent() {\n  const [showPayout, setShowPayout] = React.useState(false)"
);

// 3. Replace the alert("Retiros en desarrollo") button
text = text.replace(
...
// 4. Inject the PayoutModal at the end of DashboardContent
const modalCode = `
      {/* Payout Modal */}
      {showPayout && escrow && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            ...
            <form onSubmit={async (e) => {
              ...
              const res = await requestPayout(profile!.id, amount, method, details);
              if(res?.error) {
