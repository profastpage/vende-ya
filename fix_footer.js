...
const regex = /\{\/\* Zona 4: Footer Fijo de Accin \*\/\}[\s\S]*?<\/>\n\s*\)/;
...
const replacement = `{/* Zona 4: Footer Fijo de Acción (Estilo eBay Live) */}
          <div className="shrink-0 pt-2 pb-safe bg-gradient-to-t from-black/80 via-black/40 to-transparent">
            ...
            <div className="flex items-center gap-3 mb-2 px-3">
              {!user ? (
                <Link 
                  href="/login" 
                  className="flex-1 flex items-center justify-between px-4 h-10 border border-white/30 rounded-full bg-black/40 backdrop-blur-md transition-colors hover:bg-black/60"
                >
                  <span className="text-white/80 text-[13px] font-medium">Inicia sesión para comentar...</span>
                </Link>
              ) : (
                <div className="flex-1 flex items-center px-4 h-10 border border-white/50 rounded-full bg-black/40 backdrop-blur-md focus-within:border-white transition-colors">
                  <input
                    type="text"
              ...
              <div className="relative shrink-0 flex items-center">
                {/* Floating Emojis */}
                <div className="absolute bottom-12 right-1 pointer-events-none overflow-visible z-50">
                  <AnimatePresence>
                ...
                <button onClick={handleLike} className="h-10 w-10 flex items-center justify-center active:scale-90 transition-transform">
                  <Heart className={\`h-6 w-6 \${liked ? 'fill-rose-500 text-rose-500' : 'text-rose-500'}\`} strokeWidth={3} />
                </button>
              </div>
            </div>
            
            {/* Pequeño Banner Informativo (Opcional, estilo eBay) */}
            <div className="px-4 mb-2">
              <p className="text-[10px] text-white/60 font-medium text-center truncate">
              ...
              <div className="flex justify-between items-start mb-2.5">
                <div className="flex-1 pr-3">
                  <p className="text-white text-[13px] font-semibold leading-tight line-clamp-2">
                    {safeProduct?.title || 'Producto en Vivo'}
                  </p>
      ...
      <CheckoutBottomSheet
        ...
        shipment={{
          ...
          receiverName: user?.displayName || 'Tú',
