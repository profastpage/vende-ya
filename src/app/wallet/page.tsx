  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 pb-24 md:pb-12">
      ...
      <header className="md:hidden sticky top-0 z-30 bg-zinc-950/85 backdrop-blur-xl border-b border-white/5 pt-safe">
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-zinc-500 truncate">Mercado Pago · Billetera</p>
          </div>
        </div>
      </header>
      ...
      <div className="hidden md:block border-b border-white/5">
        <div className="max-w-3xl mx-auto px-6 py-3 flex items-center gap-1 text-sm text-zinc-500">
          <Link href={ROUTES.home} className="hover:text-zinc-200">Inicio</Link>
          <ChevronRight className="h-3 w-3 mx-1" />
          <Link href={ROUTES.dashboard} className="hover:text-zinc-200">Dashboard</Link>
          <ChevronRight className="h-3 w-3 mx-1" />
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-6 py-6 md:py-10 space-y-6">
        ...
        >
          <div className="mb-3">
            <h1 className="text-3xl md:text-4xl font-black font-display text-white">Tu billetera</h1>
        ...
        >
          {wallet.loading ? (
            <div className="rounded-3xl border border-white/5 bg-zinc-900/80 p-6 animate-pulse">
              <div className="h-4 w-1/3 bg-white/5 rounded mb-4" />
              <div className="h-10 w-1/2 bg-white/5 rounded mb-3" />
              <div className="h-3 w-2/3 bg-white/5 rounded" />
            </div>
          ) : isActive ? (
            // Active — amber gradient balance
            <div className="relative overflow-hidden rounded-3xl border border-amber-400/30 bg-gradient-to-br from-amber-500/30 via-amber-600/15 to-zinc-950 p-6 md:p-8">
              ...
              <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                  ...
                  <p className="text-4xl md:text-5xl font-black font-mono text-white tabular-nums">
                ...
                <div className="flex flex-col gap-2">
                  <button className="inline-flex items-center justify-center gap-2 h-11 px-5 rounded-xl bg-white text-zinc-950 font-bold text-sm hover:bg-zinc-100 transition-colors active:scale-95">
                    <ArrowDownToLine className="h-4 w-4" /> Retirar a MP
                  </button>
                  <button className="inline-flex items-center justify-center gap-2 h-9 px-4 rounded-lg bg-white/10 hover:bg-white/20 text-white text-xs font-medium border border-white/20 transition-colors">
                    Ver historial completo
                  </button>
                </div>
              </div>
              <div className="relative grid grid-cols-3 gap-3 mt-6 pt-6 border-t border-white/15">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-amber-200/70">En escrow</p>
                  <p className="text-lg font-bold font-mono text-white tabular-nums">{formatPEN(469.00)}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-amber-200/70">Ventas del mes</p>
                  <p className="text-lg font-bold font-mono text-white tabular-nums">{formatPEN(2314.50)}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-amber-200/70">Comisión pagada</p>
                  <p className="text-lg font-bold font-mono text-white tabular-nums">{formatPEN(277.74)}</p>
                </div>
              </div>
            </div>
          ) : (
            ...
            <div className="relative overflow-hidden rounded-3xl border border-amber-400/30 bg-amber-400/10 p-6 md:p-8">
              ...
              <div className="relative flex flex-col md:flex-row md:items-center gap-6">
                <div className="flex-1">
                  ...
                  <p className="text-2xl md:text-3xl font-black font-mono text-white/30 tabular-nums">S/. — — —</p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <h2 className="text-lg font-bold text-white">Estado de verificación</h2>
        ...
        {!wallet.loading && !isActive && (
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-white">Cómo conectar tu Mercado Pago</h2>
            </div>
            ...
            <div className="mt-5 pt-5 border-t border-white/5">
              ...
              <p className="text-xs text-zinc-500 text-center mt-3">
                ...
                <Link href={ROUTES.terminos} className="underline hover:text-zinc-300">Términos y Condiciones</Link>
                {' '}y la{' '}
        ...
        {isActive && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-xs text-zinc-400 hover:text-white underline">Ver todas</button>
            </div>
                  ...
                  {MOCK_TXS.map((tx) => (
                    <tr key={tx.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                      <td className="px-2 py-3">
                        <div className="font-medium text-white line-clamp-1">{tx.concept}</div>
                        <div className="text-xs text-zinc-500 md:hidden">{tx.buyer} · {tx.date}</div>
                        <div className="text-xs text-zinc-500 hidden md:block">{tx.date}</div>
                      </td>
                      <td className="px-2 py-3 hidden md:table-cell text-zinc-300">@{tx.buyer}</td>
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg font-bold text-white">Estructura de comisiones</h2>
          </div>
          ...
          <div className="rounded-xl bg-black/30 border border-white/10 p-4 text-sm space-y-2.5">
            <div className="flex justify-between">
              <span className="text-zinc-300">Comisión Vende Ya (en vivo)</span>
              <span className="font-mono font-bold text-amber-300">12.00%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-300">Comisión Vende Ya (marketplace)</span>
              <span className="font-mono font-bold text-amber-300">8.00%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-300">Costo pasarela (3.9% + IGV)</span>
              <span className="font-mono font-bold text-zinc-300">4.602%</span>
            </div>
            <div className="border-t border-white/10 pt-2.5 mt-1 flex justify-between font-bold">
              <span className="text-white">Neto para ti (en vivo)</span>
              <span className="text-lime-300 font-mono">83.40%</span>
            </div>
            <div className="flex justify-between font-bold">
              <span className="text-white">Neto para ti (marketplace)</span>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-white">¿Por qué vincular tu cuenta?</h2>
          </div>
          <div className="space-y-3 text-sm">
            <div className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
              <Check className="h-5 w-5 text-lime-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold text-white">Recibe pagos automáticamente</p>
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
              <Check className="h-5 w-5 text-lime-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold text-white">Split automático de comisiones</p>
                <p className="text-zinc-400 mt-0.5 leading-relaxed">
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
              <Check className="h-5 w-5 text-lime-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold text-white">Escrow protegido</p>
                <p className="text-zinc-400 mt-0.5 leading-relaxed">
...
function TxStatusBadge({ status }: { status: 'completed' | 'pending' | 'in_escrow' }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-semibold border bg-zinc-500/15 text-zinc-300 border-zinc-500/30">
      Pendiente
    </span>
  )
}
