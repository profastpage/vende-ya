...
function EmptyState() {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="rounded-3xl border border-border bg-card/60 p-8 md:p-12 text-center"
    >
      <div className="mx-auto mb-4 h-14 w-14 rounded-full bg-card flex items-center justify-center">
...
export default function LiveHubClient({ initialStreams }: { initialStreams: any[] }) {
  ...
  const liveStreams = React.useMemo(
    () => initialStreams.filter((s) => s.isLive),
    [],
  )
  const featured = liveStreams[0] ?? initialStreams[0]

  const filtered = React.useMemo(() => {
    return initialStreams.filter((s) => {
