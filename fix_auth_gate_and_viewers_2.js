...
const viewerPillOld = /function ViewersPill\(\{ viewers \}: \{ viewers: number \}\) \{[\s\S]*?\}\s*\n\s*\/\*\*/;
...
// We should also replace the loader2 import if it's not present. We can just add Loader2 to the lucide-react imports.
if (!text.includes('Loader2')) {
  text = text.replace(/import { (.*?) } from 'lucide-react'/, `import { $1, Loader2 } from 'lucide-react'`);
}
