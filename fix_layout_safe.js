...
// The main component's return statement starts right after `const youtubeUrl = ...`
const searchTarget = '`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&playsinline=1&modestbranding=1&rel=0`;';
const idx = text.indexOf(searchTarget);
if (idx === -1) {
    console.error("SEARCH TARGET NOT FOUND");
    process.exit(1);
}

const mainReturnIdx = text.indexOf('return (', idx);
const checkoutIdx = text.indexOf('<CheckoutBottomSheet', mainReturnIdx);
...
text = text.slice(0, mainReturnIdx) + newLayout + text.slice(checkoutIdx);
